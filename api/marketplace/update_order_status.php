...
try {
    ...
    // If status changed to 'paid', we update payment_status and process commission
    if ($status === 'paid' && $order['status'] !== 'paid') {
        $stmt = $pdo->prepare("UPDATE marketplace_orders SET payment_status = 'completed' WHERE id = ?");
        $stmt->execute([$order_id]);
